...
interface SpawnArgs {
  ...
  worktreeBase?: string
  /** Override the 'flt/' prefix used when naming the worktree branch. */
...
export async function spawnDirect(args: SpawnArgs): Promise<void> {
  ...
  if (useWorktree) {
    ...
    const wt = _depsForTest.createWorktree(
      baseDir,
      name,
      args.worktreeBase ?? presetWorktreeBase,
      args.worktreeBranchPrefix ?? presetWorktreeBranchPrefix,
    )
    workDir = wt.path
